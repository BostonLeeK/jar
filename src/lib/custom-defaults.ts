...
export const DEFAULT_ALERT_CSS = `.jar-alert{width:100%;color:#fff;font-family:Inter,system-ui,sans-serif;text-align:center}
.jar-alert img,.jar-gif{max-height:40cqi;max-width:100%;object-fit:contain;margin-bottom:2cqi}
.jar-line{font-size:clamp(18px,5.2cqi,56px)}
.jar-msg{margin-top:.4em;opacity:.7;font-size:clamp(13px,3.2cqi,32px)}`;
...
export const DEFAULT_GOAL_CSS = `.jar-goalw{width:100%;color:#fff;font-family:Inter,system-ui,sans-serif}
.jar-goal-row{display:flex;justify-content:space-between;gap:3cqi;font-size:clamp(14px,4.4cqi,42px);margin-bottom:2cqi}
.jar-bar{height:clamp(8px,2cqi,18px);background:rgba(255,255,255,.16);border-radius:999px;overflow:hidden}
.jar-bar i{display:block;height:100%;background:#fff}`;
...
export const DEFAULT_RECENT_CSS = `.jar-recentw{width:100%;color:#fff;font-family:Inter,system-ui,sans-serif}
.jar-rtitle{font-size:clamp(11px,3cqi,22px);letter-spacing:.16em;text-transform:uppercase;opacity:.45;margin-bottom:2cqi}
.jar-row{display:flex;justify-content:space-between;gap:3cqi;font-size:clamp(14px,4.2cqi,36px);margin:1.6cqi 0}`;
